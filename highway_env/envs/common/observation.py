...
class KinematicObservation(ObservationType):
    ...
    def space(self):
        return spaces.Box(shape=(self.vehicles_count, len(self.features)), low=-1, high=1, dtype=np.float16)
...
class SelfKinematicObservation(ObservationType):
    """
        Observe the kinematics of nearby vehicles.
    """
    FEATURES = ['presence', 'x', 'y', 'vx', 'vy']
    ...
    def normalize(self, df):
        ...
        if not self.features_range:
            ...
            self.features_range = {
                "x": [-5.0 * MDPVehicle.SPEED_MAX, 5.0 * MDPVehicle.SPEED_MAX],
                "y": [-5.0 * MDPVehicle.SPEED_MAX, 5.0 * MDPVehicle.SPEED_MAX],
                "vx": [-2*MDPVehicle.SPEED_MAX, 2*MDPVehicle.SPEED_MAX],
                "vy": [-2*MDPVehicle.SPEED_MAX, 2*MDPVehicle.SPEED_MAX]
            }
        for feature, f_range in self.features_range.items():
    ...
    def observe(self):
        df = pandas.DataFrame([])
        # Add ego-vehicle
        for vehicle in self.env.vehicle:
            df = df.append(pandas.DataFrame.from_records([vehicle.to_dict()])[self.features])
#         print(df)
        # Add nearby traffic
#         sort, see_behind = (True, False) if self.order == "sorted" else (False, True)
#         close_vehicles = self.env.road.close_vehicles_to(self.env.vehicle,
...
def observation_factory(env, config):
    ...
    elif config["type"] == "SelfKinematics":
        return SelfKinematicObservation(env, **config)
